...
class _old_VTRobokassa(VTPayment):
    def __init__(self, **kwargs):
        ...
        self.url = 'http://test.robokassa.ru/Index.aspx' if kwargs.get(
            'test_mode') else 'https://auth.robokassa.ru/Merchant/Index.aspx'
    ...
    def pay_data(self, **kwargs):
        ...
        return '%s?%s' % self.url, urlencode(
            {'MrchLogin': self.mrh_login, 'OutSum': '%.2f' % kwargs['out_sum'], 'InvId': kwargs['inv_id'],
             'Desc': kwargs.get('desc', ''), 'Culture': kwargs.get('culture', ''),
             'SignatureValue': self.crc(str_data)})
